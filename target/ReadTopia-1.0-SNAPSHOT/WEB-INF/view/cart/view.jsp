                    ...
                    <c:choose>
                        ...
                        <c:otherwise>
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-4">
                                <a href="${pageContext.request.contextPath}/homepage/book/list" class="btn btn-outline-secondary">
                                    ...
                                    <div class="d-flex flex-column gap-2" style="min-width: 250px;">
                                        ...
                                        <select class="form-select" id="voucherSelect">
                                            ...
                                            <c:forEach var="voucher" items="${vouchers}">
                                                <option value="${voucher.vouID}" data-discount="${voucher.discount}" data-quantity="${voucher.quantity}">
                                                    ${voucher.vouCode} - ${voucher.discount}% off
                                                    <c:if test="${voucher.quantity == 0}"> (Out of stock)</c:if>
                                                </option>
                                            </c:forEach>
                                        </select>
                                    </div>

                                    <input type="hidden" name="voucherID" id="voucherID" form="checkoutForm" />
...
<script>
    ...
    document.getElementById("applyPromotionBtn").addEventListener("click", function () {
        ...
        const voucher = vouchers.find(v => v.vouCode === code);
        if (!voucher) {
        ...
        voucherIDInput.value = voucher.vouID;
        ...
        codeInput.value = voucher.vouCode;
        document.querySelector(`#voucherSelect option[value="${voucher.vouID}"]`).selected = true;
        document.getElementById("promotionStatus").textContent = `Voucher Applied: ${voucher.vouName}`;
    });
